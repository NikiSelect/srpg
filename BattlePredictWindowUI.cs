...
public class BattlePredictWindowUI : MonoBehaviour
{
    //スクリプト連携
    ...
    //UI表示更新
    public void UpdateBattlePredictWindow(Character playerChara, Character enemyChara)
    {
        ...
        int playerNowHpValue = battleManager.getNowHpValue(playerChara, enemyDamageValue);
        int enemyNowHpValue = battleManager.getNowHpValue(enemyChara, playerDamageValue);
        ...
        txtPlayerHp.text = $"{playerNowHpValue} / {playerChara.maxHP}";
        ...
        float playerHpfillAmout = Mathf.Clamp((float)playerNowHpValue / (float)playerChara.maxHP, 0f, 1f);
        ...
        txtEnemyHp.text = $"{enemyNowHpValue} / {enemyChara.maxHP}";
        ...
        float enemyHpfillAmout = Mathf.Clamp((float)enemyNowHpValue / (float)enemyChara.maxHP, 0f, 1f);
